...
class SFlagBaseTest(ImmunizationBaseTest):
    ...
    def store_imms(
        ...
        patient_is_restricted: bool,
        imms_identifier_value=None,
    ) -> dict:
        """
        ...
        """
        nhs_number = (
            valid_nhs_number_with_s_flag if patient_is_restricted else valid_nhs_number1
        )
    ...
    def test_get_not_s_flagged_imms(self):
        ...
        for imms_api in self.imms_apis:
            with self.subTest(imms_api):
                ...
                read_imms = imms_api.get_immunization_by_id(imms).json(
                    parse_float=Decimal
                )
...
class TestSearchSFlagImmunization(SFlagBaseTest):
        ...
        for imms_api in self.imms_apis:
            with self.subTest(imms_api):
                ...
                response = imms_api.search_immunizations(
                    valid_nhs_number_with_s_flag, VaccineTypes.covid_19
                ).json(parse_float=Decimal)
                # Then
                hit_imms = self.filter_my_imms_from_search_result(
                    response, imms1, imms2
                )
                for hit_imm in hit_imms:
                    ...
                    expected_response["patient"]["reference"] = hit_imm["patient"][
                        "reference"
                    ]
                    expected_response["identifier"][0]["value"] = hit_imm["identifier"][
                        0
                    ]["value"]
    ...
    def filter_my_imms_from_search_result(search_body: dict, *my_ids) -> List[dict]:
        ...
        return [
            entry["resource"]
            for entry in search_body["entry"]
            if entry["resource"]["id"] in my_ids
        ]
