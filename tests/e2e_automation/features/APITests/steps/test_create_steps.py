...
@given(
    "Valid json payload is created where vaccination terms has multiple instance of coding with different coding system"
)
def createValidJsonPayloadWithProcedureMultipleCodingsDifferentSystem(context):
    ...
    context.immunization_object.extension[0].valueCodeableConcept.coding[
        0
    ].system = "http://example.com/different-system"
...
def createValidJsonPayloadWithProcedureNoTextValue(context):
    valid_json_payload_is_created(context)
    context.immunization_object.extension[0].valueCodeableConcept = build_vaccine_procedure_code(
        context.vaccine_type.upper(), add_extensions=False
    )
...
def validate_imms_event_table(context):
    ...
    fields_to_compare = [
        (
            "IdentifierPK",
            f"{create_obj.identifier[0].system}#{create_obj.identifier[0].value}",
...
def validate_procedure_term_text_in_delta_table(context):
    ...
    assert actual_terms["procedure_term"] == context.create_object.extension[0].valueCodeableConcept.text, (
        f"Expected procedure term '{context.create_object.extension[0].valueCodeableConcept.text}', but got '{actual_terms['procedure_term']}'"
    )
    assert actual_terms["product_term"] == context.create_object.vaccineCode.text, (
        f"Expected product term '{context.create_object.vaccineCode.text}', but got '{actual_terms['product_term']}'"
    )
    assert actual_terms["site_term"] == context.create_object.site.text, (
        f"Expected site of vaccination term '{context.create_object.site.text}', but got '{actual_terms['site_term']}'"
    )
    assert actual_terms["route_term"] == context.create_object.route.text, (
        f"Expected route of vaccination term '{context.create_object.route.text}', but got '{actual_terms['route_term']}'"
    )
    print(
...
def validate_procedure_term_first_display_in_delta_table(context):
    actual_terms = get_all_term_text(context)
    assert actual_terms["procedure_term"] == context.create_object.extension[0].valueCodeableConcept.coding[0].display, (
        f"Expected procedure term '{context.create_object.extension[0].valueCodeableConcept.coding[0].display}', but got '{actual_terms['procedure_term']}'"
    )
    assert actual_terms["product_term"] == context.create_object.vaccineCode.coding[0].display, (
        f"Expected product term '{context.create_object.vaccineCode.coding[0].display}', but got '{actual_terms['product_term']}'"
    )
...
def validate_procedure_term_correct_coding_in_delta_table(context):
    actual_terms = get_all_term_text(context)
    assert actual_terms["procedure_term"] == context.create_object.extension[0].valueCodeableConcept.coding[1].display, (
        f"Expected procedure term text '{context.create_object.extension[0].valueCodeableConcept.coding[1].display}', but got '{actual_terms['procedure_term']}'"
    )
    assert actual_terms["product_term"] == context.create_object.vaccineCode.coding[1].display, (
        f"Expected product term '{context.create_object.vaccineCode.coding[1].display}', but got '{actual_terms['product_term']}'"
    )
    assert actual_terms["site_term"] == context.create_object.site.coding[1].display, (
        f"Expected site of vaccination term '{context.create_object.site.coding[1].display}', but got '{actual_terms['site_term']}'"
    )
    assert actual_terms["route_term"] == context.create_object.route.coding[1].display, (
        f"Expected route of vaccination term '{context.create_object.route.coding[1].display}', but got '{actual_terms['route_term']}'"
    )
...
def validate_procedure_term_second_display_in_delta_table(context):
    actual_terms = get_all_term_text(context)
    assert actual_terms["procedure_term"] == context.create_object.extension[0].valueCodeableConcept.coding[0].display, (
        f"Expected procedure term text '{context.create_object.extension[0].valueCodeableConcept.coding[0].display}', but got '{actual_terms['procedure_term']}'"
    )
    assert actual_terms["product_term"] == context.create_object.vaccineCode.coding[0].display, (
        f"Expected product term text '{context.create_object.vaccineCode.coding[0].display}', but got '{actual_terms['product_term']}'"
    )
    assert actual_terms["site_term"] == context.create_object.site.coding[0].display, (
        f"Expected site of vaccination term text '{context.create_object.site.coding[0].display}', but got '{actual_terms['site_term']}'"
    )
    assert actual_terms["route_term"] == context.create_object.route.coding[0].display, (
        f"Expected route of vaccination term text '{context.create_object.route.coding[0].display}', but got '{actual_terms['route_term']}'"
    )
...
def validate_procedure_term_blank_in_delta_table(context):
    actual_terms = get_all_term_text(context)
    assert actual_terms["procedure_term"] == "", (
        f"Expected procedure term text to be blank, but got '{actual_terms['procedure_term']}'"
    )
    assert actual_terms["product_term"] == "", (
        f"Expected product term text to be blank, but got '{actual_terms['product_term']}'"
    )
    assert actual_terms["site_term"] == "", (
        f"Expected site of vaccination term text to be blank, but got '{actual_terms['site_term']}'"
    )
    assert actual_terms["route_term"] == "", (
        f"Expected route of vaccination term text to be blank, but got '{actual_terms['route_term']}'"
    )
...
def create_request_with_invalid_gender(context, gender):
    valid_json_payload_is_created(context)
