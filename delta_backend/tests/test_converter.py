...
class TestConverter(unittest.TestCase):
    ...
    def test_runConversion_schemaParserException(
        self, mock_getSchemaParser, mock_getFHIRParser
    ):
        converter = Converter({})
    ...
    @patch.object(Converter, "_getFHIRParser")
    @patch.object(Converter, "_getSchemaParser")
    def test_runConversion_conversionCheckerException(
        self, mock_getSchemaParser, mock_getFHIRParser
    ):
        converter = Converter({})
        mock_getFHIRParser.return_value = MagicMock()
        ...
        with patch(
            ...
            side_effect=RuntimeError("Mocked Checker failure"),
        ):
            result = converter.runConversion(json_data={})
            self.assertEqual(result["code"], 0)
    ...
    @patch.object(Converter, "_getFHIRParser")
    @patch.object(Converter, "_getSchemaParser")
    def test_runConversion_getConversionsException(
        self, mock_getSchemaParser, mock_getFHIRParser
    ):
        converter = Converter({})
        mock_getFHIRParser.return_value = MagicMock()
        schema_mock = MagicMock()
        schema_mock.getConversions.side_effect = RuntimeError(
            "Mocked getConversions failure"
        )
    ...
    def test_runConversion_success(
        self, mock_checker, mock_schema_parser, mock_fhir_parser
    ):
        converter = Converter({})
        ...
        result = converter.runConversion(
            json_data={"occurrenceDateTime": "2023-01-01T12:00:00"}
        )
    ...
    def test_extract_patient_details(
        self, mock_practitioner, mock_site_code, mock_patient, mock_address, mock_names
    ):
        converter = Converter({})
        json_data = {
        ...
        result = converter.extract_patient_details(
            json_data, "PERFORMING_PROFESSIONAL_SURNAME"
        )
