...
def get_patient(json_data):
    ...
    return next(
        (
            c
            for c in contained
            if isinstance(c, dict) and c.get("resourceType") == "Patient"
        ),
        None,
    )
...
def get_valid_names(names, occurrence_time):
    official_names = [
        n
        for n in names
        if n.get("use") == "official" and is_current_period(n, occurrence_time)
    ]
    if official_names:
    ...
    valid_names = [
        n
        for n in names
        if is_current_period(n, occurrence_time) and n.get("use") != "old"
    ]
...
def get_valid_address(patient, occurrence_time):
    ...
    valid_addresses = [
        a
        for a in addresses
        if "postalCode" in a and is_current_period(a, occurrence_time)
    ]
    ...
    selected_address = next(
        (
            ...
            if a.get("use") == "home" and a.get("type") != "postal"
        ),
        next(
            (
                a
                for a in valid_addresses
                if a.get("use") != "old" and a.get("type") != "postal"
            ),
            next(
                (a for a in valid_addresses if a.get("use") != "old"),
                valid_addresses[0],
            ),
        ),
    )
...
def extract_site_code(json_data):
    ...
    valid_performers = [
        p for p in performers if "actor" in p and "identifier" in p["actor"]
    ]
    ...
    selected_performer = next(
        (
            ...
            and p.get("actor", {}).get("identifier", {}).get("system")
...
def extract_practitioner_names(json_data, occurrence_time):
    ...
    practitioner = next(
        (
            c
            for c in contained
            if isinstance(c, dict) and c.get("resourceType") == "Practitioner"
        ),
        None,
    )
    ...
    valid_practitioner_names = [
        n for n in practitioner_names if "given" in n or "family" in n
    ]
    if not valid_practitioner_names:
        return "", ""

    selected_practitioner_name = get_valid_names(
        valid_practitioner_names, occurrence_time
    )
    performing_professional_forename = " ".join(
        selected_practitioner_name.get("given", [])
    )
...
def is_current_period(name, occurrence_time):
    ...
    return (not start or start <= occurrence_time) and (
        not end or occurrence_time <= end
    )
