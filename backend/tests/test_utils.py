...
from src.models.utils.validation_utils import (
...
class TestGenericUtils(unittest.TestCase):
    ...
    def test_convert_disease_codes_to_vaccine_type(self):
        ...
        for combination, vaccine_type in valid_combinations:
            self.assertEqual(
                convert_disease_codes_to_vaccine_type(combination), vaccine_type
            )
    ...
    def test_get_vaccine_type(self):
        ...
        for vaccine_type in vaccine_types:
            valid_json_data = load_json_data(
                filename=f"completed_{vaccine_type.lower()}_immunization_event.json"
            )
        ...
        flu_json_data = load_json_data(
            filename=f"completed_{VaccineTypes.flu.lower()}_immunization_event.json"
        )
        valid_target_disease_element = {
            "coding": [
                ...
                },
                {
                    ...
                    "code": f"{DiseaseCodes.flu}",
                    "display": "Influenza",
                },
            ]
        }
        flu_json_data["protocolApplied"][0]["targetDisease"][
            0
        ] = valid_target_disease_element
        ...
        for invalid_target_disease in invalid_target_disease_elements:
            invalid_covid_19_json_data = deepcopy(covid_19_json_data)
            invalid_covid_19_json_data["protocolApplied"][0]["targetDisease"][
                0
        ...
        mmr_json_data = load_json_data(
            filename=f"completed_{VaccineTypes.mmr.lower()}_immunization_event.json"
        )
        ...
        for invalid_target_disease in invalid_target_disease_elements:
            invalid_mmr_json_data = deepcopy(mmr_json_data)
            invalid_mmr_json_data["protocolApplied"][0]["targetDisease"][
                1
            ] = invalid_target_disease
            with self.assertRaises(ValueError) as error:
                get_vaccine_type(invalid_mmr_json_data)
