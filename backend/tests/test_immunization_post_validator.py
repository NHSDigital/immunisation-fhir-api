...
class TestImmunizationModelPostValidationRules(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.completed_json_data = {
            VaccineTypes.covid_19: load_json_data(
    ...
    def test_collected_errors(self):
        ...
        for performer in covid_19_json_data["performer"]:
            if performer["actor"].get("type") == "Organization":
                if (
                    performer["actor"]["identifier"].get("system")
    ...
    def test_post_validate_and_set_vaccine_type(self):
        ...
        """
        all_target_disease_codes_field_location = "protocolApplied[0].targetDisease[*].coding[?(@.system=='http://snomed.info/sct')].code"
        first_target_disease_code_field_location = "protocolApplied[0].targetDisease[0].coding[?(@.system=='http://snomed.info/sct')].code"
        ...
        for vaccine_type in [
            ...
            self.assertIsNone(
                self.validator.validate(self.completed_json_data[vaccine_type])
            )
        ...
        expected_error_message = (
            f"{first_target_disease_code_field_location} is a mandatory field"
        )
    ...
    def test_post_patient_name_given(self):
        ...
        invalid_json_data = parse(patient_name_field_location).filter(
            lambda d: True, deepcopy(valid_json_data)
        )
        ...
        json_data = update_contained_resource_field(
            json_data, "Patient", "name", valid_name_array
        )
        ...
        json_data = deepcopy(valid_json_data)
        json_data = update_contained_resource_field(
            json_data, "Patient", "name", invalid_name_array
        )
        with self.assertRaises(ValueError) as error:
    ...
    def test_post_patient_name_family(self):
        ...
        expected_error_message = (
            f"{patient_name_family_field_location} is a mandatory field"
        )
        ...
        # Case 2: One name instance with no given field fails validation
        MandationTests.test_missing_mandatory_field_rejected(
            self, patient_name_family_field_location
        )
    ...
    def test_post_practitioner_name_given(self):
        ...
        json_data = deepcopy(valid_json_data)
        json_data = update_contained_resource_field(
            json_data, "Practitioner", "name", invalid_name_array
        )
        MandationTests.test_present_field_accepted(self, json_data)
    ...
    def test_post_practitioner_name_family(self):
        ...
        MandationTests.test_missing_field_accepted(
            self, field_location=practitioner_name_family_field_location
        )
    ...
    def test_post_dose_number_positive_int(self):
        ...
        """
        dose_number_positive_int_field_location = (
        ...
        for vaccine_type in self.all_vaccine_types:
            ...
            MandationTests.test_missing_field_accepted(
                self, dose_number_positive_int_field_location, valid_json_data
            )
    ...
    def test_post_manufacturer_display(self):
        ...
        for vaccine_type in self.all_vaccine_types:
            MandationTests.test_missing_field_accepted(
                self, field_location, self.completed_json_data[vaccine_type]
            )
    ...
    def test_post_no_snomed_code(self):
        ...
        covid_19_json_data["protocolApplied"][0][
            "targetDisease"
        ] = invalid_target_disease_value
