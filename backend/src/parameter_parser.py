...
def process_params(aws_event: APIGatewayProxyEventV1) -> ParamContainer:
    ...
    def parse_multi_value_query_parameters(
        ...
        params = [
            (k, split_and_flatten(v)) for k, v in multi_value_query_params.items()
        ]
    ...
    query_params = parse_multi_value_query_parameters(
        aws_event.get("multiValueQueryStringParameters", {}) or {}
    )
...
def process_search_params(params: ParamContainer) -> SearchParams:
    ...
    patient_identifier = (
        patient_identifiers[0] if len(patient_identifiers) == 1 else None
    )

    if patient_identifier is None:
        raise ParameterException(
            f"Search parameter {patient_identifier_key} must have one value."
        )

    patient_identifier_parts = patient_identifier.split("|")
    if (
        len(patient_identifier_parts) != 2
        or not patient_identifier_parts[0] == patient_identifier_system
    ):
        raise ParameterException(
    ...
    vaccine_types = [
        vaccine_type
        for vaccine_type in params[immunization_target_key]
        if vaccine_type is not None
    ]
    if len(vaccine_types) < 1:
        raise ParameterException(
            f"Search parameter {immunization_target_key} must have one or more values."
        )
    ...
    if len(date_froms) > 1:
        raise ParameterException(
            f"Search parameter {date_from_key} may have one value at most."
        )

    try:
        date_from = (
            datetime.datetime.strptime(date_froms[0], "%Y-%m-%d").date()
            if len(date_froms) == 1
            else date_from_default
        )
    except ValueError:
        raise ParameterException(
            f"Search parameter {date_from_key} must be in format: YYYY-MM-DD"
        )
    ...
    if len(date_tos) > 1:
        raise ParameterException(
            f"Search parameter {date_to_key} may have one value at most."
        )

    try:
        date_to = (
            datetime.datetime.strptime(date_tos[0], "%Y-%m-%d").date()
            if len(date_tos) == 1
            else date_to_default
        )
    except ValueError:
        raise ParameterException(
            f"Search parameter {date_to_key} must be in format: YYYY-MM-DD"
        )
