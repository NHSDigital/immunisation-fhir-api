...
def process_params(aws_event: APIGatewayProxyEventV1) -> ParamContainer:
    ...
    def parse_multi_value_query_parameters(multi_value_query_params: dict[str, list[str]]) -> ParamContainer:
...
def process_search_params(params: ParamContainer) -> SearchParams:
    ...
    if len(patient_identifier_parts) != 2 or not patient_identifier_parts[0] == patient_identifier_system:
    ...
    vaccine_types = [vaccine_type for vaccine_type in params[immunization_target_key] if vaccine_type is not None]
...
def create_query_string(search_params: SearchParams) -> str:
    params = [
        ...
        (patient_identifier_key, f"{patient_identifier_system}|{search_params.patient_identifier}"),
        *(
