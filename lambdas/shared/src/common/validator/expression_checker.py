...
class ExpressionChecker:
        ...
        try:
            if field_value != expression_rule:
                raise RecordError(ExceptionMessages.RECORD_CHECK_FAILED, "Value equals check failed",
                                  "Value does not equal expected value, Expected- " + expression_rule + " found- "
                                  + field_value)
        except RecordError as e:
            code = e.code if e.code is not None else ExceptionMessages.RECORD_CHECK_FAILED
            message = (
        ...
        try:
            if field_value == expression_rule:
                raise RecordError(ExceptionMessages.RECORD_CHECK_FAILED, "Value not equals check failed",
                                  "Value equals expected value when it should not, Expected- " + expression_rule
                                  + " found- " + field_value)
        except RecordError as e:
            code = e.code if e.code is not None else ExceptionMessages.RECORD_CHECK_FAILED
            message = (e.message if e.message is not None
        ...
        try:
            if expression_rule.lower() in field_value.lower():
                raise RecordError(ExceptionMessages.RECORD_CHECK_FAILED,
                                  ...
                                  + expression_rule + " Data- " + field_value)
        except RecordError as e:
            code = e.code if e.code is not None else ExceptionMessages.RECORD_CHECK_FAILED
            message = (e.message if e.message is not None
                       else ExceptionMessages.MESSAGES[ExceptionMessages.RECORD_CHECK_FAILED])
    ...
    def _validate_upper(self, expression_rule, field_name, field_value, row) -> ErrorReport:
        try:
            ...
            if not result:
                raise RecordError(ExceptionMessages.RECORD_CHECK_FAILED,
                                  "Value not uppercase", "Check Value not found to be uppercase, value- " + field_value)
        except RecordError as e:
            code = e.code if e.code is not None else ExceptionMessages.RECORD_CHECK_FAILED
            message = (e.message if e.message is not None
                       else ExceptionMessages.MESSAGES[ExceptionMessages.RECORD_CHECK_FAILED])
        ...
        try:
            ...
            if not result:
                raise RecordError(ExceptionMessages.RECORD_CHECK_FAILED,
                                  ...
                                  "Value does not start as expected, Expected- " + expression_rule
                                  + " found- " + field_value)
        except RecordError as e:
            code = e.code if e.code is not None else ExceptionMessages.RECORD_CHECK_FAILED
            message = (e.message if e.message is not None
        ...
        try:
            ...
            if not result:
                raise RecordError(ExceptionMessages.RECORD_CHECK_FAILED,
                                  ...
                                  "Value does not end as expected, Expected- " + expression_rule
                                  + " found- " + field_value)
        except RecordError as e:
            code = e.code if e.code is not None else ExceptionMessages.RECORD_CHECK_FAILED
            message = (e.message if e.message is not None
