        ...
        try:
            ...
            if 'T' in iso_datetime_str :
                ...
                if '+' in iso_datetime_str :
                    ...
                    dt_obj = datetime.strptime(timestamp_str_with_colon, "%Y-%m-%dT%H:%M:%S%z")
                else:
...
class SampleModel(BaseModel, Constants):
    NHS_NUMBER: str = None
    ...
    def validate_person_gender_code(cls, value):
        if value:
            if value not in {'0','1', '2', '9'}:
                raise ValueError("Invalid value for PERSON_GENDER_CODE. It must be 0, 1, 2, or 9.")
            return value
    ...
    def validate_action_flag(cls, value):
        if not value:
            ...
            if not (value in ["completed", "entered-in-error"]):
                raise ValueError("ACTION_FLAG shouled be 'completed' or 'entered-in-error' ")
            else:
                return value
    ...
    def validate_professional_forename(cls, v, values):
        ...
        if v :
            if not surname:
               raise ValueError("If PERFORMING_PROFESSIONAL_FORENAME is given, PERFORMING_PROFESSIONAL_SURNAME must also be given")
            else:
                return v
        else:
            if surname:
                raise ValueError("If PERFORMING_PROFESSIONAL_SURNAME is given, PERFORMING_PROFESSIONAL_FORENAME must also be given")
    ...
    def validate_professional_reg_code(cls, v, values):
        if v :
            ...
            #if is_empty(reg_uri):
            if not reg_uri :
               raise ValueError("If PERFORMING_PROFESSIONAL_BODY_REG_CODE is given, PERFORMING_PROFESSIONAL_BODY_REG_URI must also be given")
            else:
                return v
    ...
    def validate_report_origin(cls, v, values):
        ...
        if (primary_source) and (not v) :
            raise ValueError("REPORT_ORIGIN is a mandatory field, when PRIMARY_SOURCE is given")  
        return v
    
    @validator("NOT_GIVEN", pre=True, always=True)
    def validate_not_given_flag(cls, value,values):
        if value:
                if not (value == Constants.vaccination_not_given_flag or value == Constants.vaccination_given_flag):
    ...
    def validate_vaccination_procedure_code(cls, v, values) :
        if "NOT_GIVEN" in values :
                if not Constants.if_vaccine_not_give(values["NOT_GIVEN"]):
                    if not v:
                        raise ValueError("VACCINATION_PROCEDURE_CODE is a mandatory field, when NOT_GIVEN=FALSE")
        return v
    ...
    def validate_vaccination_situatuion_code(cls, v, values) :
        if "NOT_GIVEN" in values :
                if Constants.if_vaccine_not_give(values["NOT_GIVEN"]):
                    if not v:
                        raise ValueError("VACCINATION_SITUATION_CODE is a mandatory field, when NOT_GIVEN=TRUE")
        return v
    ...
    def validate_reason_not_given_code(cls, v, values) :
        if "NOT_GIVEN" in values :
                if Constants.if_vaccine_not_give(values["NOT_GIVEN"]):
                    if not v:
                        raise ValueError("REASON_NOT_GIVEN_CODE is a mandatory field, when NOT_GIVEN=TRUE") 
        return v
    ...
    def validate_dose_sequence(cls, v, values) :
        if "NOT_GIVEN" in values :
                if not Constants.if_vaccine_not_give(values["NOT_GIVEN"]):
                    if not v:
                        raise ValueError("DOSE_SEQUENCE is a mandatory field, when NOT_GIVEN=FALSE")
                else:
                    return 1
        return v
    ...
    def validate_vaccine_product_code(cls, v, values) :
        if "NOT_GIVEN" in values :
                if not Constants.if_vaccine_not_give(values["NOT_GIVEN"]):
                    if not v:
                        raise ValueError("VACCINE_PRODUCT_CODE is a mandatory field, when NOT_GIVEN=FALSE")
                else:
                    return 1
        return v
    ...
    def validate_vaccine_manufacturer(cls, v, values) :
        if "NOT_GIVEN" in values :
                if not Constants.if_vaccine_not_give(values["NOT_GIVEN"]):
                    if not v:
                        raise ValueError("VACCINE_MANUFACTURER is a mandatory field, when NOT_GIVEN=FALSE")
        return v
    ...
    def validate_batch_number(cls, v, values) :
        if "NOT_GIVEN" in values :
                if not Constants.if_vaccine_not_give(values["NOT_GIVEN"]):
                    if not v:
                        raise ValueError("BATCH_NUMBER is a mandatory field, when NOT_GIVEN=FALSE") 
        return v 
    ...
    def validate_expiry_date(cls, v,values):
         if "NOT_GIVEN" in values :
            if not Constants.if_vaccine_not_give(values["NOT_GIVEN"]):
                if not v:
                    raise ValueError("EXPIRY_DATE is a mandatory field, when NOT_GIVEN=FALSE")
                else:
    ...
    def validate_route_of_vaccination_code(cls, v, values) :
        ...
        if "NOT_GIVEN" in values :
                if not Constants.if_vaccine_not_give(values["NOT_GIVEN"]):
                    if not v:
                        raise ValueError("ROUTE_OF_VACCINATION_CODE is a mandatory field, when NOT_GIVEN=FALSE") 
        return v       
    ...
    def validate_dose_amount(cls, v, values) :
        ...
        if "NOT_GIVEN" in values :
                if not Constants.if_vaccine_not_give(values["NOT_GIVEN"]):
                    if not v:
                        raise ValueError("DOSE_AMOUNT is a mandatory field, when NOT_GIVEN=FALSE")
                    else:
                        if not (Constants.has_max_decimal_places(v)):
                            raise ValueError("DOSE_AMOUNT should have maximum 4 decimal places")
                        return v 
        return None         
    ...
    def validate_dose_unit_code(cls, v, values) :
        ...
        if "NOT_GIVEN" in values :
                if not Constants.if_vaccine_not_give(values["NOT_GIVEN"]):
                    if not v:
                        raise ValueError("DOSE_UNIT_CODE is a mandatory field, when NOT_GIVEN=FALSE")  
        return v   
    ...
    def validate_indication_code(cls, v, values) :
        ...
        if "NOT_GIVEN" in values :
                if not Constants.if_vaccine_not_give(values["NOT_GIVEN"]):
                    if not v:
                        raise ValueError("INDICATION_CODE is a mandatory field, when NOT_GIVEN=FALSE")  
        return v   
    ...
    def validate_consent_for_treatment_code(cls, v, values) :
        ...
        if "NOT_GIVEN" in values :
                if not Constants.if_vaccine_not_give(values["NOT_GIVEN"]):
                    if not v:
                        raise ValueError("CONSENT_FOR_TREATMENT_CODE is a mandatory field, when NOT_GIVEN=FALSE")  
        return v   
