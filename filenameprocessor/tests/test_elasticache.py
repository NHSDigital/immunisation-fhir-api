...
class TestElasticache(TestCase):
    ...
    def test_permissions_caching(self):
        ...
        """
        mock_permissions_1 = {
            "test_supplier_1": ["RSV_FULL"],
            "test_supplier_2": ["FLU_CREATE", "FLU_UPDATE"],
        }
        mock_permissions_config_1 = generate_permissions_config_content(
            mock_permissions_1
        )
        ...
        mock_permissions_config_2 = generate_permissions_config_content(
            mock_permissions_2
        )
        ...
        with patch("elasticache.redis_client", fakeredis.FakeStrictRedis()):
            ...
            s3_client.put_object(
                Bucket=BucketNames.CONFIG,
                Key=PERMISSIONS_CONFIG_FILE_KEY,
                Body=mock_permissions_config_2,
            )
            upload_to_elasticache(PERMISSIONS_CONFIG_FILE_KEY, BucketNames.CONFIG)
            self.assertEqual(
                get_permissions_config_json_from_cache(),
                {"all_permissions": mock_permissions_2},
            )
