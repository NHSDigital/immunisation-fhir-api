...
resource "aws_route53_record" "grafana_validation" {
  for_each = {
    for dvo in aws_acm_certificate.grafana_url_certificate.domain_validation_options : dvo.domain_name => {
...
resource "aws_acm_certificate_validation" "grafana" {
  certificate_arn         = aws_acm_certificate.grafana_url_certificate.arn
